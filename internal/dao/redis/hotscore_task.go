...
// NewHotScoreRefresher 创建热度刷新器实例
func NewHotScoreRefresher(rdb *goredis.Client) *HotScoreRefresher {
...
func (r *HotScoreRefresher) batchRefreshGravityScores(ctx context.Context, postIDs []string) error {
	...
	for _, postID := range postIDs {
		...
		if communityID != "" {
			pipe.ZAdd(ctx, redisKey(keyCommunityPostScorePrefix+communityID), goredis.Z{
				Score:  score,
				Member: postID,
			})
		}
