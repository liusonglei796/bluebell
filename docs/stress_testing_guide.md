...
*记下返回的 token，后续测试中将作为 `Authorization` 头使用。*
...
**参数说明:** 
- `-c`: 并发连接数 (Connections)
- `-t`: 线程数 (Threads) [go-wrk 中通常不需要像 wrk 那样关注线程，主要关注 -c]
- `-n`: 总请求数 (Requests)
- `-d`: 测试持续时间 (Duration)
- `-H`: HTTP Header
...
**目标接口:** 获取帖子列表 (`GET /api/v1/posts2`)
**测试目的:** 评估 Redis (ZSet) + MySQL (批量查询) 的混合性能。这是系统中最核心的高频接口。

```bash
# 模拟 200 个并发连接，持续 20 秒
go-wrk -c 200 -d 20s -H "Authorization: Bearer <YOUR_TOKEN>" http://127.0.0.1:8080/api/v1/posts2?page=1&size=10&order=score
```
...
```bash
# 注意：POST 请求需要 body，go-wrk 支持 -b 参数
# 模拟 50 个并发 (bcrypt 计算很慢，并发过高会导致超时)
go-wrk -c 50 -d 10s -M POST -b '{"username":"testuser","password":"yourpassword"}' -H "Content-Type: application/json" http://127.0.0.1:8080/api/v1/login
```
...
Running 10s test @ http://127.0.0.1:8080/api/v1/community
  ...
  Requests/sec:		1458.98  <-- 核心指标：RPS (每秒请求数)
