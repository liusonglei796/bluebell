# Bluebell 项目教学文档 (DDD/Clean Architecture 版)
...
**重构日期:** 2026年3月
**项目版本:** v3.0 (DDD/Clean Architecture)
...
2. **DDD/Clean Architecture**: 分层架构、依赖注入、Repository 接口、UnitOfWork
3. **数据库操作**: GORM ORM、数据建模、查询优化、Preload 关联
4. **缓存应用**: Redis 数据结构、缓存策略、分布式场景
5. **认证授权**: JWT 认证、Refresh Token、会话管理
6. **业务建模**: 用户系统、社区管理、帖子发布、投票机制
7. **工程实践**: 日志系统、配置管理、API 文档、错误处理
...
| 03 | [用户注册业务流程设计](./03-用户注册业务流程设计.md) | DDD分层架构、依赖注入、Repository接口 |
| 04 | [请求参数绑定与校验](./04-请求参数绑定与校验.md) | ShouldBindJSON、validator标签 |
| 05 | [优雅的参数校验与错误翻译](./05-优雅的参数校验与错误翻译.md) | 自定义校验规则、错误信息中文化 |
| 06 | [用户密码加密与数据持久化](./06-用户密码加密与数据持久化.md) | bcrypt密码哈希、GORM BeforeCreate钩子 |
| 07 | [Zap日志系统集成与环境分离](./07-Zap日志系统集成与环境分离.md) | 结构化日志、日志分级、Lumberjack轮转 |
| 08 | [JWT认证与登录功能实现](./08-JWT认证与登录功能实现.md) | JWT原理、Service层DI、中间件依赖注入 |
...
1. **基础阶段 (第1-7章)**: 必须完整学习,掌握用户系统核心功能
2. **认证阶段 (第8-12章)**: 理解 JWT 认证、日志系统和开发工具
3. **业务阶段 (第13-18章)**: 重点关注社区管理、帖子发布和 Redis 应用
...
**教学文档更新:**
- 第01、03、08章: 修复与最新源码不一致的文件路径名称、配置项结构以及 DI 装配代码
- 第15-18章: 全面重写为使用 Redis 实现帖子特性排序分页和按社区筛选的高性能架构

### v3.0 (2026-03-02)

**架构升级:**
- 重构为 DDD/Clean Architecture：Handler-Service-DAO 分层 + 依赖注入
- 引入 Repository 接口和 UnitOfWork 模式
- 错误处理迁移到 `errorx.CodeError` 统一模式
- 密码加密从 DAO 层移到 Model `BeforeCreate()` 钩子
- 中间件改为依赖注入风格 `JWTAuthMiddleware(tokenCache)`
- Entry point 迁移到 `cmd/bluebell/main.go`

**教学文档更新:**
- 第03章: 全面重写为 DDD 分层架构讲解
- 第06章: 密码加密更新为 Model BeforeCreate 钩子
- 第08章: 登录、中间件、路由更新为依赖注入风格
- 第13章: 社区功能更新为 Repository 模式
- 第14章: 帖子功能更新为 Service/DAO DI 风格
- 第16章: 投票功能更新为 errorx 错误处理
