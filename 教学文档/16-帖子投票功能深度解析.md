...
// 错误码和消息分离,容易不一致
const CodeVoteTimeExpire ResCode = 5001
var codeMsgMap = map[int]string{CodeVoteTimeExpire: "投票时间已过"}

// ✅ 新方式: errorx.CodeError 结构体
// 错误码和消息绑定在一起,不会不一致
var ErrVoteTimeExpire = errorx.New(1009, "投票时间已过")
```
...
前端发送 POST /api/v1/vote
        ↓
[Handler] 参数校验 (PostID + Direction)
        ↓
[Handler] 获取用户ID (JWT中间件)
        ↓
[Service] 通过依赖注入调用投票逻辑
        ↓
[Redis Cache] 检查帖子发布时间
        ↓
    时间>7天? ─Yes→ 返回 errorx.ErrVoteTimeExpire
        ↓No
[Redis Cache] 查询用户之前的投票记录
        ↓
   新旧值相同? ─Yes→ 返回 errorx.ErrVoteRepeated
        ↓No
[Redis Cache] 计算分数变化
        ↓
[Redis Pipeline] 原子更新:
