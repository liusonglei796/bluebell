...
// InitTrans 初始化翻译器
// 为什么: validator 默认的错误提示是英文,为了提升用户体验,需要配置中文翻译
func InitTrans() (err error) {

    // 🔥 步骤1: 获取 Validator 引擎并进行类型断言
...
errs := map[string]string{
    "SignUpRequest.username": "username为必填字段",
    "SignUpRequest.password": "password长度必须至少为6个字符",
}
...
func SignUpHandler(c *gin.Context) {
    ...
    if err := c.ShouldBindJSON(&p); err != nil {
        ...
        if !ok {
            // 非校验错误(如 JSON 格式错误)
            backfront.ResponseError(c, CodeInvalidParam)
            return
        }

        // 🔥 核心步骤
        // 1. errs.Translate(myvalidator.Trans) - 翻译成中文
        // 2. myvalidator.RemoveTopStruct() - 去除结构体名前缀
        // 3. ResponseErrorWithMsg() - 返回优化后的错误信息
        backfront.ResponseErrorWithMsg(c, CodeInvalidParam, myvalidator.RemoveTopStruct(errs.Translate(myvalidator.Trans)))
        return
    }
...
func SignUpHandler(c *gin.Context) {
    ...
    if err := c.ShouldBindJSON(&p); err != nil {
        ...
        // 4️⃣ 翻译并优化错误信息
        translatedErrs := errs.Translate(myvalidator.Trans)       // 翻译成中文
        optimizedErrs := myvalidator.RemoveTopStruct(translatedErrs)  // 去除前缀

        // 5️⃣ 返回详细错误
        backfront.ResponseErrorWithMsg(c, errorx.CodeInvalidParam, optimizedErrs)
        return
    }
    ...
    if err := logic.SignUp(&p); err != nil {
        ...
        // 根据业务错误码返回不同错误，使用 errorx.GetCode 判断
        if errorx.GetCode(err) == errorx.CodeUserExist {
            backfront.ResponseError(c, errorx.CodeUserExist)
            return
        }

        backfront.ResponseError(c, errorx.CodeServerBusy)
        return
    }
