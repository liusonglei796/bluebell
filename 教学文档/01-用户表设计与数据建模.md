...
CREATE TABLE `user` (
    ...
    -- 认证字段
    -- ============================================
    `user_name` VARCHAR(64) COLLATE utf8mb4_general_ci NOT NULL COMMENT '用户名 (登录凭证)',
    `passwd` VARCHAR(255) COLLATE utf8mb4_general_ci NOT NULL COMMENT '密码 (bcrypt 哈希值)',
...
**实现位置:** `internal/infrastructure/snowflake/snowflake.go`

详细实现请参考 [第02章: Snowflake 算法生成分布式 ID](./02-Snowflake算法生成分布式ID.md)

---

### 3.3 认证字段: `user_name` 和 `passwd`
...
- VARCHAR(255) 预留了充足空间,兼容未来可能的加密算法升级
...
```sql
`role` TINYINT(4) NOT NULL DEFAULT 1 COMMENT '用户角色 (1=普通用户, 2=管理员)',
```

**设计说明:**
- 使用 `TINYINT` 存储角色类型,高效且节约
- 默认值为 `1` (普通用户),确保新注册用户具有基本权限
- 管理员角色 (`2`) 具有特殊权限,如创建社区、删除不当内容等
...
if user.Role != model.RoleAdmin {
    return errorx.ErrForbidden // 403 禁止访问
}
```

---

### 3.6 时间戳: `create_time` 和 `update_time`
...
type User struct {
	...
	Role     int    `gorm:"column:role;default:1;not null"` // 1: 普通用户, 2: 管理员
}
...
**注意事项:**
1. 使用 `gorm` 标签映射数据库字段名 (原 sqlx 使用 `db` 标签)
2. `BcryptCost = 10` 控制加密强度，每增加 1 计算时间翻倍
3. `TableName()` 方法用于自定义表名，GORM 默认使用复数形式
4. Go 类型与 MySQL 类型的对应:
   - `BIGINT` → `int64`
   - `VARCHAR` → `string`
...
1. **双 ID 设计 (针对 User 和 Post)**:
   - `id` 用于数据库内部(自增、高效)
   - `user_id`/`post_id` 用于业务层(安全、不可预测，雪花算法生成)

2. **单 ID 策略 (针对 Community)**:
   - 社区模型直接使用 `id` 自增主键作为标识符。
   - **理由**：社区数量相对有限，不存在高频遍历攻击的风险，且直接使用主键能显著提升关联查询性能。
