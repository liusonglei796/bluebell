<template>
  <nav class="fixed top-4 left-0 right-0 z-50 px-4">
    <div class="max-w-7xl mx-auto glass rounded-full shadow-lg" style="background: rgba(255, 255, 255, 0.8);">
      <div class="px-6">
        <div class="flex justify-between h-16">
          <div class="flex">
            <router-link to="/" class="flex-shrink-0 flex items-center">
              <span class="text-xl font-black tracking-tighter text-black">BLUEBELL</span>
            </router-link>
            ...
            <div class="hidden sm:ml-8 sm:flex sm:items-center">
              <form @submit.prevent="handleSearch" class="relative">
                <input 
                  ...
                  type="text" 
                  class="block w-64 pl-4 pr-10 py-2 border border-black/10 rounded-full leading-5 bg-white/50 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-black/20 sm:text-sm backdrop-blur-sm transition-all" 
          ...
          <div class="flex items-center space-x-6">
            <template v-if="authStore.token">
              ...
              <button @click="logout" class="text-sm font-bold text-gray-500 hover:text-black transition-colors">
                Logout
              </button>
